...
- Releases publish via npm Trusted Publishing (OIDC) from GitHub Actions — no npm token in CI.
...
- Initial public release, extracted from the internal monorepo where the tool was built and
  ...
  - `@evgentus/visual-feedback` — framework-agnostic dev-only overlay (shadow DOM, comment queue with
    localStorage persistence, element→source resolver, connection indicator, screenshot
    annotation via html2canvas).
  - `@evgentus/visual-feedback/vite` — Vite plugin that tags JSX with `data-vf-source="file:line:col"`
    (the React path; `apply: 'serve'`, zero production footprint).
  - `@evgentus/visual-feedback/nuxt` — Nuxt 4 module; self-disables in production builds.
  - `@evgentus/visual-feedback/react` — `<VisualFeedback />` adapter.
  - `@evgentus/visual-feedback/mcp` — programmatic API of the channel server (`handleBatch`,
